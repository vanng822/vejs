...
function filtered(js) {
	return js.substr(1).split('|').reduce(function(js, filter){
		var parts = filter.split(':')
				, name = parts.shift()
				, args = parts.shift();
		return 'filters.' + name + '(' + js + (args? ', ' + args : '') + ')';
	});
};

var fixOnlineComment = function(str) {
	var str = str.split('<script');
	var i, len, lines;
	for (i = 0, len = str.length; i < len; i++) {
		lines = str[i].split('</script');
		lines[0] = lines[0].split('\n').map(function(line) {
			return line.replace(/\/\/(.+)/,'/* $1 */')
		}).join('\n');;
		str[i] = lines.join('</script');
	}
	return str.join('<script');
};
